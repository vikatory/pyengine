...
void PyEngine::init()
{
	...
	cout << sCurPath << endl;
	// 通过设置环境变量包含dll路径
	SetEnviromentValue("PYTHONPATH", "E:\\CODE\\project_1\\pyx\\pyx\\PyQt5;E:\\CODE\\project_1\\python-3.5.0-embed-amd64;E:\\CODE\\project_1\\python-3.5.0-embed-amd64\\Lib;");
	AddEnviromentValue("PATH", "E:\\CODE\\project_1\\pyx\\pyx\\PyQt5");
	wchar_t sPyProgramName[20] = L"python35";
	Py_SetProgramName(sPyProgramName);
...
void PyEngine::test()
{
	// TODO: 考虑怎样与boost.log结合, py.log优化
	string sPyPath = scriptPath();
	exec(("g_ScriptRoot = \"" + sPyPath + "\"\n").c_str());
	sPyPath.append("/main.py");
	exec("g_PyLibPath = \"E:\\CODE\\project_1\\python-3.5.0-embed-amd64\\Lib\"");
	string sAppPath = "g_AppPath = \"" + GetCurrentPath() + "\"";
	exec(sAppPath.c_str());
